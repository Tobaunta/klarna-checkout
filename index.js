...
app.get('/', async (req,res) => {
    ...
    const markup = products.map((p) => `<a style="display: block; color: black; border: solid 2px black; margin: 10px; padding: 10px" href="/products/${p.id}">${p.title} - ${p.price}kr</a>`).join(' ');
    res.send(markup);
})
...
app.get('/products/:id', async (req, res) => {
    try {
        ...
        res.send(klarnaResponse.html_snippet);
    } catch(error) {
